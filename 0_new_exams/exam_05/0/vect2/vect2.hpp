...
public:
    ...
    //vect2& operator=(const vect2& oth) { if (*this != oth) { x = oth.x; y = oth.y; } return *this; }
    // int& operator[](int i) { return (i == 0 ? x : y); }
    // int operator[](int i) const { return (i == 0 ? x : y); }
    // friend std::ostream& operator<<(std::ostream& os, const vect2& v) { os << "{" << v[0] << ", " << v[1] << "}"; return os; }
    ...
    // bool operator==(const vect2& oth) { return (x == oth.x && y == oth.y); }
    // bool operator!=(const vect2& oth) { return !(x == oth.x && y == oth.y); }
};