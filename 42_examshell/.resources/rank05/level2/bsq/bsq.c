<<<<<<< HEAD
#include <unistd.h>
#include <stdio.h>
#include <stdlib.h>
#include <string.h>

int min3(int a, int b, int c) {return a < b ? (a < c ? a : c) : (b < c ? b : c);}
void clean_map(char **map, int h, FILE *file)
{
    fprintf(stderr, "error map\n");
    for (int i=0; i < h; i++)
        if(map && map[i]) free(map[i]);
=======
#include <string.h>
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>

int min3(int a, int b, int c) {return a < b ? (a < c ? a : c) : (b < c ? b : c);}
void clean_map(char **map, int h, FILE *file){
    fprintf(stderr, "map error\n");
    for(int i=0;i<h;i++)
        if(map && map[i])
            free(map[i]);
>>>>>>> 1adbcdf (exam06)
    free(map);
    if (file != stdin)
        fclose(file);
    return;
}
void process_file(char *filename){
<<<<<<< HEAD
    int h=0, w=0, max=0, maxi=0, maxj=0, *buffer;
    char empty, obstacle, full, **map;
    size_t len =0;
    ssize_t read = 0;

    FILE *file = filename ? fopen(filename, "r") : stdin;
    if(!file) {fprintf(stderr, "error map\n"); return;}
    if(fscanf(file, "%d %c %c %c\n", &h, &empty, &obstacle, &full) != 4 || h <= 0)
    {
        fprintf(stderr, "error map\n");
        if (file != stdin)
            fclose(file);
        return;
    }
    map = calloc((size_t)h, sizeof(char*));
    if(!map) {clean_map(map, h, file); return;}
    for(int i = 0; i < h; i++ ) {
        read = getline(&map[i], &len, file);
        if(read == -1) {clean_map(map, h, file); return;}
        if (map[i][read -1] == '\n') map[i][read -1] = 0;
        if (i == 0) w = strlen(map[i]);
        if((int)strlen(map[i]) != w) {clean_map(map, h, file); return;}
        for (int j = 0; j < w; j++)
        if(map[i][j] != empty && map[i][j] != obstacle) {clean_map(map, h, file); return;}
    }
    
    if (empty == obstacle || empty == full || full == obstacle) {clean_map(map, h, file); return;}
    
    buffer = calloc((size_t)h*w, sizeof(int));
    if(!buffer) {clean_map(map, h, file); return;}
    for(int i = 0; i < h; i++ ) {
        for(int j = 0; j < w; j++){
            if (map[i][j] == obstacle) buffer[i * w + j] = 0;
            else if (i == 0 || j == 0) buffer[i * w + j] = 1;
            else buffer[i * w + j] = min3(buffer[(i - 1) * w + j], buffer[i * w + (j - 1)], buffer[(i - 1) * w + (j - 1)]) + 1;
            if( buffer[i * w + j] > max){
                max =  buffer[i * w + j];
=======
    int h=0,w=0,max=0,maxi=0,maxj=0,*buffer;
    char empty, obstacle, full, **map;
    size_t len=0;
    ssize_t read = 0;
    FILE *file = filename ? fopen(filename, "r") : stdin;
    // if(!file) {fprintf(stderr, "map error\n"); return;}
    if(fscanf(file, "%d %c %c %c\n", &h, &empty, &obstacle,&full) !=4 || h <=0) {fprintf(stderr, "map error\n"); return;}
    map = calloc((size_t)h, sizeof(char*));
    if(!map)  {clean_map(map, h, file); return;}
    for(int i=0;i<h;i++){
        read = getline(&map[i], &len, file);
        if(read == -1) {clean_map(map, h, file); return;}
        if(map[i][read-1] == '\n') map[i][read-1] = 0;
        if(i==0) w= strlen(map[i]);
        if((int)strlen(map[i]) != w) {clean_map(map, h, file); return;}
        for(int j=0;j<w;j++)
            if(map[i][j] != empty && map[i][j] != obstacle) {clean_map(map, h, file); return;}
    }
    if (empty == obstacle || empty == full || obstacle == full) {clean_map(map, h, file); return;}
    buffer = calloc((size_t)h*w, sizeof(int));
    if(!buffer)  {clean_map(map, h, file); return;}

    for(int i=0;i<h;i++){
        for(int j=0;j<w;j++){
            if(map[i][j] == obstacle) buffer[i*w+j] = 0;
            else if(i==0 || j==0) buffer[i*w+j] = 1;
            else buffer[i*w+j] = 1+ min3(buffer[(i-1)*w+j], buffer[i*w+(j-1)], buffer[(i-1)*w+(j-1)]);
            if(buffer[i*w+j] > max) {
                max = buffer[i*w+j];
>>>>>>> 1adbcdf (exam06)
                maxi = i;
                maxj = j;
            }
        }
    }
<<<<<<< HEAD
    for (int i = maxi - max + 1; i < maxi; i++)
        for (int j = maxj - max + 1; j < maxj; j++)
            map[i][j] = full;

    for(int i = 0; i < h; i++ )
        puts(map[i]);
    for(int i = 0; i < h; i++ )
        free(map[i]);
    free(map); free(buffer);
=======

    for(int i=maxi - max +1;i<maxi;i++)
        for(int j=maxj-max+1;j<maxj;j++)
            map[i][j] = full;





    for(int i=0;i<h;i++)
        puts(map[i]);
            
    for(int i=0;i<h;i++)
        free(map[i]);
    free(map);
    free(buffer);

    if(file != stdin)
        fclose(file);
    return ;
>>>>>>> 1adbcdf (exam06)

    if (file != stdin)
        fclose(file);

    return;

        
}

<<<<<<< HEAD
// int main(int ac, char **av){
//     if(ac == 1)
//         process_file(NULL);
//     else for (int i=1; i <= ac; i++)
//         process_file(av[i]);
=======
// int main(int ac, char **av)
// {
//     if (ac == 1) process_file(NULL);
//     else for (int i=1;i<ac;i++) process_file(av[i]);
>>>>>>> 1adbcdf (exam06)
//     return 0;
// }