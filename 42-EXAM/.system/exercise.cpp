...
bool exam::prepare_current_ex(void)
{
    ...
    cmd_system_call = "cp " + get_path() + "* .system/grading/ 2>/dev/null";
    system(cmd_system_call.c_str());
    
    // ensure tester.sh is executable
    system("chmod +x .system/grading/tester.sh 2>/dev/null");
    
    // Debug: Check if tester.sh was copied
    if (!file_exists(".system/grading/tester.sh")) {
        std::cout << "Warning: tester.sh not found at " << get_path() << std::endl;
        // Try to find tester.sh in the exercise directory
        std::string alt_path = get_path() + "tester.sh";
        if (file_exists(alt_path)) {
            std::cout << "Found tester.sh, copying..." << std::endl;
            cmd_system_call = "cp " + alt_path + " .system/grading/";
            system(cmd_system_call.c_str());
            system("chmod +x .system/grading/tester.sh");
        }
    }
