...
std::map<int, exercise> exam::list_dir(void)
{
    ...
    // Check for new exam structure first
    if (student && exam_number == 5)
    {
        std::string new_path = "0_new_exams/exam_0" + std::to_string(exam_number) + "/" + std::to_string(level) + "/";
...
std::string exam::get_path(void)
{
    ...
    if (student && exam_number == 5)
    {
        std::string new_path = "0_new_exams/exam_0" + std::to_string(exam_number) + "/" + std::to_string(level) + "/" + current_ex->get_name() + "/";
        DIR *new_dir = opendir(new_path.c_str());
        if (new_dir != NULL)
        {
            path_exam = new_path;
...
void exam::set_max_lvl(void)
{
    if (student)
    {
        if (exam_number == 2)
            ...
            level_max = 2; // Updated for new exam structure (levels 0, 1, 2)
